...
type Opportunity = { segment: { weekday: string; start_hour: number; end_hour: number; }; estimate: { value_low: Money; value_high: Money; assumptions: string[]; } | null; confidence: number; };
  ...
  useEffect(() => {
    async function loadDashboard() {
      try {
        ...
        if (opportunitiesResponse.ok) { setOpportunity((await opportunitiesResponse.json() as Opportunity[])[0] ?? null); }
        setMessage("");
