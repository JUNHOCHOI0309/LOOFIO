...
type Opportunity = {
  ...
  scoring: {
    version: string;
    breakdown: { impact: number; confidence: number; persistence: number; actionability: number; total: number; } | null;
  };
  confidence: number;
  limitations: string[];
...
const supportsRecommendation = (opportunity: Opportunity | null) => opportunity !== null;
const actionTitlePlaceholder = (opportunity: Opportunity | null) => {
  if (opportunity?.type === "CANCELLATION_HOTSPOT") return "예: 금요일 저녁 예약 이탈 운영 점검";
  if (opportunity?.type === "DORMANT_CUSTOMER") return "예: 재방문 지연 코호트 운영 기록 점검";
  if (opportunity?.type === "SERVICE_DEMAND_GAP") return "예: 화요일 오후 Offering 예약 상태 점검";
  return "예: 화요일 오후 수동 혜택 실험";
};
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {metrics && metrics.appointment_count > 0 && <>
        ...
        {opportunities.length > 0 && <article className="opportunity-card opportunity-list-card"><div className="card-title"><div><h2>저장된 Opportunity</h2><p>Detector가 생성·갱신한 후보입니다. Observation과 Estimate, Recommendation을 구분해 표시합니다.</p></div><span className="opportunity-count">{opportunities.length}개</span></div><div className="opportunity-list">{opportunities.map((item) => <button className={item.id === opportunity?.id ? "selected" : ""} key={item.id} onClick={() => void selectOpportunity(item)}><div><span>{opportunityTypeLabel[item.type]}</span><strong>{opportunitySummary(item)}</strong><small>우선순위 {item.score.toFixed(1)} · {item.detector.version} · 신뢰도 {(item.confidence * 100).toFixed(0)}%</small></div><div className="opportunity-list-estimate">{item.estimate ? <><b>Estimate</b><strong>{money(item.estimate.value_low)}–{money(item.estimate.value_high)}</strong></> : <><b>Observation</b><strong>추정 없음</strong></>}</div></button>)}</div><p className="safety">✓ 모든 유형은 수동 검토용 Recommendation만 만들 수 있습니다. 원인·매출 손실·외부 실행을 뜻하지 않습니다.</p></article>}
        {opportunity?.scoring.breakdown && <article className="opportunity-card score-card"><div className="card-title"><div><h2>Opportunity Score · {opportunity.score.toFixed(1)}</h2><p>서로 다른 기회 후보의 검토 순서를 정하는 deterministic 우선순위 점수입니다.</p></div><span className="opportunity-count">{opportunity.scoring.version}</span></div><div className="score-breakdown"><div><span>Impact</span><strong>{opportunity.scoring.breakdown.impact.toFixed(1)} / 35</strong></div><div><span>Confidence</span><strong>{opportunity.scoring.breakdown.confidence.toFixed(1)} / 30</strong></div><div><span>Persistence</span><strong>{opportunity.scoring.breakdown.persistence.toFixed(1)} / 20</strong></div><div><span>Actionability</span><strong>{opportunity.scoring.breakdown.actionability.toFixed(1)} / 15</strong></div></div><p className="safety">✓ 점수는 우선순위 비교용이며 성공 확률, 예상 매출 또는 Action 효과가 아닙니다.</p></article>}
