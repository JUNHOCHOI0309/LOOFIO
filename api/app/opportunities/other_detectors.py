...
def _cancellation_drafts(rows: list[AppointmentMetricRow]) -> list[OpportunityDraft]:
    ...
    for candidate in detection.candidates:
        ...
        drafts.append(OpportunityDraft(
            ...
            estimate=None, score=round(min(75, 20 + candidate.rate_multiple * 15 + candidate.appointment_count * 0.2), 2),
            confidence=round(min(0.7, 0.35 + candidate.observed_weeks * 0.015 + candidate.appointment_count * 0.005), 3), limitations=detection.limitations,
...
def _service_demand_drafts(rows: list[AppointmentMetricRow]) -> list[OpportunityDraft]:
    ...
    for candidate in detection.candidates:
        ...
        drafts.append(OpportunityDraft(
            ...
            estimate=None, score=round(min(70, 20 + (1 - candidate.share_index) * 35 + candidate.slot_appointment_count * 0.2), 2),
            confidence=round(min(0.7, 0.3 + candidate.observed_weeks * 0.015 + candidate.slot_appointment_count * 0.005), 3), limitations=detection.limitations,
...
def _dormant_customer_drafts(rows: list[AppointmentMetricRow], *, as_of_date: date) -> list[OpportunityDraft]:
    ...
    for candidate in detection.candidates:
        ...
        drafts.append(OpportunityDraft(
            ...
            estimate=None, score=round(min(70, 15 + candidate.overdue_ratio * 8 + candidate.completed_visit_count * 2), 2),
            confidence=round(min(0.7, 0.3 + candidate.reference_interval_count * 0.025), 3), limitations=detection.limitations,
